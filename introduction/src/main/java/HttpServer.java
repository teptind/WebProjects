...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        String utf8Uri = URLDecoder.decode(request.getUri(), "UTF-8");
        File file = new File(root, utf8Uri);
        if (file.isDirectory()) {
            utf8Uri += "/index.html";
            file = new File(root, utf8Uri);
        }
        if (file.isFile()) {
            byte[] body = readFile(file);
            String hashCode = String.valueOf(Arrays.hashCode(body));
            if (!hashCode.equals(request.getHeader("If-None-Match"))) {
